		...
		if (requestTokenHeader != null) {
			if (requestTokenHeader.startsWith("Bearer ")) {
				...
				try {
					...
					return AuthErrorDto.jwtError(response, 401, EAuthError.EXPIRED_JWT, "JWT has old structure: " + e.message)
				} catch (e: MalformedJwtException) {
					return AuthErrorDto.jwtError(response, 401, EAuthError.INVALID_JWT, "JWT has invalid form")
				} catch (e: IllegalArgumentException) {
					return AuthErrorDto.jwtError(response, 401, EAuthError.INVALID_JWT,
							"Could not extract JWT data: " + e.message)
				} catch (e: SignatureException) {
					return AuthErrorDto.jwtError(response, 401, EAuthError.INVALID_JWT, "Invalid JWT signature")
				} catch (e: UsernameNotFoundException) {
					return AuthErrorDto.jwtError(response, 401, EAuthError.INVALID_JWT, e.message!!)
				}
			} else {
				return AuthErrorDto.jwtError(response, 401, EAuthError.INVALID_JWT, "JWT does not begin with Bearer")
			}
		}
