...
import ch.judos.snakes.region.core.helper.ResponseEntityHelper
...
class AuthenticationAdminController @Autowired constructor(
	...
	fun createAuthenticationToken(@RequestBody request: LoginRequestDto): ResponseEntity<Any> {
		if (this.loginAttemptService.isBlocked(ELoginType.ADMIN, request.username)) {
			return ResponseEntityHelper.tooManyAttempts()
		}
		return try {
			...
			return ResponseEntityHelper.disabledUser()
		} catch (e: LockedException) {
			return ResponseEntityHelper.disabledUser()
		} catch (e: BadCredentialsException) {
			return ResponseEntityHelper.invalidCredentials()
		}
	}
